...
namespace PlatformerPro
{
    ...
    /// </summary>
    public class LeftRightMovement : Platform
    {
        /// <summary>
        ...
        override protected void Init()
        {
            ...
            bottomExtent = transform.position.x - bottomOffset;
            topExtent = transform.position.x + topOffset;
        }
        ...
        protected virtual void DoMove()
        {
            if (speed > 0)
            {
                ...
                if (myTransform.position.x >= topExtent)
                {
                    speed *= -1;
                ...
                else
                {
                    ...
                    if (myTransform.position.x > topExtent)
                    {
                        float difference = distance - (myTransform.position.x - topExtent);
                        myTransform.position = new Vector3(topExtent - difference, myTransform.position.y,  myTransform.position.z);
                        speed *= -1;
                        if (automaticDeactivation == PlatformDeactivationType.DEACTIVATE_ON_EXTENT) Activated = false;
                    }
                }
            ...
            {
                if (myTransform.position.x <= bottomExtent)
                {
                    speed *= -1;
                    if (automaticDeactivation == PlatformDeactivationType.DEACTIVATE_ON_EXTENT) Activated = false;
                }
                else
                {
                    ...
                    if (myTransform.position.x < bottomExtent)
                    {
                        float difference = distance - (myTransform.position.x - bottomExtent);
                        myTransform.position = new Vector3(bottomExtent - difference, myTransform.position.y,  myTransform.position.z);
                        speed *= -1;
                        if (automaticDeactivation == PlatformDeactivationType.DEACTIVATE_ON_EXTENT) Activated = false;
                    }
                }
        ...
        void OnDrawGizmos()
        {
            ...
            else if (GetComponent<Collider2D>() is BoxCollider2D)
            {
                ...
                right = boxCollider.size.x / 2.0f;
                left = boxCollider.size.x / -2.0f;
            }

            Gizmos.color = Platform.GizmoColor;
            Gizmos.DrawLine(transform.position, transform.position + new Vector3(topOffset + right,0 , 0));
            Gizmos.DrawLine(transform.position + new Vector3(topOffset + right,0.25f,  0), transform.position + new Vector3(topOffset + right, -0.25f, 0));
            Gizmos.DrawLine(transform.position, transform.position + new Vector3(-bottomOffset + left, 0, 0));
            Gizmos.DrawLine(transform.position + new Vector3(-bottomOffset + left, 0.25f, 0), transform.position + new Vector3(-bottomOffset + left, -0.25f,  0));
        }
    }
